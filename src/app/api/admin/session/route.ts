...
function cookieOpts(maxAgeSec: number) {
  ...
  return `HttpOnly; Path=/; Max-Age=${maxAgeSec}; SameSite=Lax${secure ? "; Secure" : ""}`;
}
...
export async function POST(req: Request) {
  ...
  res.headers.append("Set-Cookie", `admin_token=${encodeURIComponent(token)}; ${cookieOpts(7 * 24 * 60 * 60)}`);
  return res;
}